...
const TaskList = ({ userEmail }) => {
    ...
    const fetchTasks = async () => {
        try {
            const response = await axios.get(`http://localhost:4000/api/tasks/${userEmail}`);
    ...
    const updateDueDate = async (taskId, deadline) => {
        try {
            await axios.put("http://localhost:4000/api/tasks/updateDueDate", { taskId, deadline });
            fetchTasks();
        } catch (error) {
    ...
    const moveTask = async (taskId, direction) => {
        try {
            await axios.put("http://localhost:4000/api/tasks/reorder", { userEmail, taskId, direction });
            fetchTasks();
        } catch (error) {
    ...
    const completeTask = async (taskId) => {
        try {
            await axios.put("http://localhost:4000/api/tasks/complete", { taskId });
            fetchTasks();
        } catch (error) {
